                ...
                return (
                  <div 
                    ...
                    className={`rounded-2xl shadow-sm border overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 relative transition-colors duration-500
                        ${order.status === OrderStatus.CANCELLED ? 'bg-red-50 border-red-100' : 'bg-white'}
                        ${ageStatus === 'WARNING' ? 'border-yellow-300 ring-1 ring-yellow-200' : ''}
                        ${ageStatus === 'CRITICAL' ? 'border-red-300 ring-1 ring-red-200 bg-red-50/30' : ''}
                        ${ageStatus === 'NORMAL' && order.status !== OrderStatus.CANCELLED ? 'border-slate-200' : ''}
                    `}
                  >
                     {/* Status Bar */}
                          ...
                          <div className="flex items-center gap-2 mt-1">
                             ...
                             {order.status !== 'DELIVERED' && order.status !== 'CANCELLED' && (
                                 <div className={`text-[10px] font-mono px-1.5 py-0.5 rounded flex items-center gap-1 ${
                                    ageStatus === 'CRITICAL' ? 'bg-red-100 text-red-600 font-bold animate-pulse' :
                                    ageStatus === 'WARNING' ? 'bg-yellow-100 text-yellow-700 font-bold' :
                        ...
                        {order.status === OrderStatus.NEW && (
                          <div className="flex gap-2">
                             <button 
                                ...
                                className="px-3 py-3 bg-red-50 hover:bg-red-100 text-red-500 border border-red-200 rounded-xl"
                              ...
                              <button 
                                ...
                                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-100 active:scale-[0.98] transition-all"
                              >
